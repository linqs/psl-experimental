...
public abstract class AbstractHitAndRunSampler implements Sampler {
	...
	protected abstract void processNewDimension(AtomFunctionVariable var, int index);

	protected abstract double sampleAlpha(Matrix direction, Matrix Aobj, Matrix objConst, double alphaLow, double alphaHigh);

	protected abstract void processSampledPoint(Iterable<GroundRule> groundRules);
	...
	public void sample(Iterable<GroundRule> groundRules, double activationThreshold, int activatorThreshold) {
			...
			if (r instanceof WeightedGroundRule) {
				...
				switch(con.getComparator()) {
					...
					case SmallerThan:
						...
						break;
					default:
						throw new AssertionError("Unknown comparator type: " + con.getComparator());
				}
			} else {
				throw new AssertionError("Unknown evidence type: " + r);
			}
		...
		for (GroundRule e : groundRules) {
			if (e instanceof WeightedGroundRule) {
				...
				FunctionTerm corefun = getCoreObjectiveFunction(ev.getFunctionDefinition());
				if (corefun == null) {
					corefun = ev.getFunctionDefinition();
				}

				if (!corefun.isLinear()) {
					throw new AssertionError("Expected linear probabilistic evidence only, but got: " + ev);
				}
				...
				if (!con.getFunction().isLinear()) {
					throw new AssertionError("Expected linear constraints only, but got: " + ev);
				}
		...
		Matrix projection;
		if (noEqConstraints==0) {
			projection = MatrixFactory.eye(dimensions,dimensions);
		} else {
			...
			//Determine rank
			int eqRank = 0;
			while (eqRank<noEqConstraints && svd[1].getAsDouble(eqRank,eqRank) != 0.0) {
				eqRank++; //TODO: is checking for 0.0 ok here, or rather |x|<epsilon?
			}
		...
		if (dimensions < maxDimension2Display) {
			log.trace("P {}",Arrays.toString(projection.getSize()));
		}
		...
		do {
			if ((noSteps + 1) % 10000 == 0) {
				log.debug("Starting step #{}", noSteps + 1);
			} else {
				log.trace("Starting step #{}", noSteps + 1);
			}
			...
			if (inCorner) {
				...
				for (int i=0; i < noActiveCons; i++) {
					smaller.setAsDouble(-Math.abs(dimGenerator.nextDouble()), i,0);
				}
				...
				for (int i = 0; i < reducedDim; i++) {
					direction.setAsDouble(dimGenerator.nextGaussian(), i,0);
				}
			}

			double dnorm = 0.0;
			for (int i=0;i<reducedDim;i++) {
				dnorm += Math.pow(direction.getAsDouble(i,0),2.0);
			}
	...
	private double setMatrixRow(Matrix m, int row, FunctionTerm term,boolean negate) {
		double constant = 0.0;
		if (term instanceof FunctionSummand) {
			constant = setMatrixCell(m,row,(FunctionSummand)term,negate);
		} else {
			if (!(term instanceof FunctionSum)) {
				throw new IllegalArgumentException("Expected sum but was given: " + term);
			}
	...
	private double setMatrixCell(Matrix m, int row, FunctionSummand summand,boolean negate) {
		if (summand.isConstant()) {
			return summand.getValue();
		} else {
			//Add entry to matrix
			if (!(summand.getTerm() instanceof AtomFunctionVariable)) {
				throw new IllegalArgumentException("Expected sum of simple variables but got: " + summand);
			}
	...
	*/
	private static FunctionTerm getCoreObjectiveFunction(FunctionTerm fun) {
